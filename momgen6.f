...
C Code to compute the four times correlation function    
     ...
     +        datan((1+sq2*x)/(sq2-x*x))/pi
      ...
      idum = -(jdum*1000+kdum*100)-values(3)
...
c      idum = -53
      ...
      tgrade = t0
!      ncut = 3000
C genera anzitutto la cumulativa
      xmin = 1.d2
      dpn = xmin/ng
c      dpn = 1.d-2
      xmin = -dpn*ng
...
C Genera secondo 1/(1+x**4)
               ...
               enddo
               state = 
     +          dpn*(il*(pn(iu)-xc)+iu*(xc-pn(il)))/(pn(iu)-pn(il))
            endif
         enddo
C We have the time sequence, we are ready to compute correlations
         n4 = nd/8
         do k = 1,nf
            if (k.le.6) then
               i1 = 1
            ...
            do i4 = i1+ncut,nd
               ...
               i6 = nint(i1+r6*(i4-i1))
               xfr(i4,k) = xfr(i4,k) + 
     +          xin(i1)*xin(i2)*xin(i3)*xin(i4)*xin(i5)*xin(i6)
               if(iave.eq.1) write(200+k,*) i1,i2,i3,i4,i5,i6 
               if(iave.eq.1) write(300+k,*) dt*(i1-1),dt*(i5-1),
     +                                   dt*(i2-1),dt*(i3-1),
     +                                   dt*(i5-1),dt*(i4-1)
            enddo
            close(200+k)
